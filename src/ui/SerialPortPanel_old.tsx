...
export const SerialPortPanel: React.FC<SerialPortPanelProps> = ({ className }) => {
  ...
  const [serialData, setSerialData] = useState<string[]>([]);  const [sendMessage, setSendMessage] = useState<string>('');
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [error, setError] = useState<string>('');
  const [isHexMode, setIsHexMode] = useState<boolean>(false);const [config, setConfig] = useState<SerialPortConfig>({
    baudRate: 115200,
    dataBits: 8,
    stopBits: 1,
    parity: 'none'
  ...
  const addToDataDisplay = useCallback((message: string, type: 'system' | 'sent' | 'received' | 'error' = 'system', skipTimestamp = false) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeDisconnected = window.electron.onSerialDisconnected(() => {
      ...
      // 使用新格式：[时间戳]:data
      const displayText = isHexMode 
        ? `${data.timestamp}:${data.hexData}` 
        : `${data.timestamp}:${data.data}`;
      addToDataDisplay(displayText, 'received', true);
    });
  ...
  }, [refreshPorts, checkConnectionStatus, addToDataDisplay, isHexMode]);
  const handleConnect = async () => {
    ...
    try {
      setIsLoading(true);
      setError('');
      addToDataDisplay(`Attempting to connect to ${selectedPort} with baud rate ${config.baudRate}...`, 'system');
      
      const success = await window.electron.connectSerialPort(selectedPort, config);
      if (!success) {
        setError('Failed to connect to serial port - unknown error');
        addToDataDisplay('Connection failed - unknown error', 'error');
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : String(err);
      setError(errorMessage);
      addToDataDisplay(`Connection failed: ${errorMessage}`, 'error');
      
      // 如果是错误代码31，给出更多建议
      if (errorMessage.includes('Unknown error code 31') || errorMessage.includes('Port is busy')) {
        addToDataDisplay('Tip: Try refreshing ports or using a different baud rate', 'system');
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <div className={`serial-port-panel ${className || ''}`}>
      <div className="serial-port-connection">
        ...
        <div className="connection-config">
          <div className="config-row">
            <label htmlFor="baud-rate">Baud Rate:</label>            <select
              id="baud-rate"
      ...
      <div className="serial-port-communication">
        <h3>Serial Communication</h3>
          ...
          <div className="data-display" ref={dataDisplayRef}>            {serialData.length === 0 ? (
